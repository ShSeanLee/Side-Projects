...
def get_target_price(code="005930"):
    """변동성 돌파 전략으로 매수 목표가 조회""" # by 래리 윌리엄스
    ...
    target_price = stck_oprc + (stck_hgpr - stck_lwpr) * 0.5 # 오늘 시가 + (전일고가 - 전일 저가) * k(0.5)
    return target_price
...
try:
    ...
    while True:
        ...
        if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
            ...
            break
        if t_9 < t_now < t_start and soldout == False: # 잔여 수량 매도, 처음 장 시작할대 보유주식이 없게
